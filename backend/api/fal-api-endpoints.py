...
async def generate_wan25(
    ...
    try:
        ...
        generation.result_url = result.get("image", {}).get("url")
...
async def edit_with_qwen(
    ...
    try:
        ...
        generation = Generation(
            ...
            prompt=request_data.instruction,
            parameters=request_data.dict(),
            status="processing",
            created_at=datetime.utcnow(),
            project_id=request_data.project_id
...
async def generate_product_shots(
    ...
    try:
        ...
        generation = Generation(
            ...
            prompt=f"{request_data.product_category}: {request_data.product_description}",
            parameters=request_data.dict(),
            status="processing",
            created_at=datetime.utcnow(),
            project_id=request_data.project_id
        ...
        # Update generation record
        generation.status = "completed"
        generation.result_urls = result.get("images", [])
        generation.completed_at = datetime.utcnow()
        db.commit()
